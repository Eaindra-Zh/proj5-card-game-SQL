...
/*Getting to know max, min and avg score of each game*/
SELECT game_name, 
MAX(score) AS max_score, 
MIN(score) AS min_score, 
AVG(score) AS avg_score 
FROM card_games GROUP BY game_name 
ORDER BY max_score desc;

/*The highest scorer of each game*/
select game_name, player_name, max(score) as max_score
from card_games group by game_name
order by max_score desc;

/*Assigning player category to a game*/
select game_name, player_name, score, 
CASE
        when score = 314 THEN "Legendary player"
        when score >= round(180.75) THEN "Excellent player"
        when score > 91 THEN "Average player"
        else "Bad player"
    END as "player_cat"
from card_games where game_name LIKE "crazy eights"
order by score desc;

/*Most recently played game for each player*/
select date_played, game_name, player_name
